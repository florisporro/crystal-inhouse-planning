...
| Moved-in status | Set manually by apartment owners (not inferred). Residents choose between "no move planned yet", "move planned" and "fully moved in"; "no response" (never provided info — chase them) is the default and admin-only. Activity creation runs through the `/announce` wizard. |
