...
| Moved-in status | Residents set "no move planned yet" or "fully moved in" directly; "move planned" is reached **only** by announcing a moving activity with the "fully moved in after this move" checkbox (default on) — future date ⇒ planned + date, past date ⇒ moved in. Adjusting/cancelling that activity keeps/clears the status. A planned move whose date has passed automatically reads as "moved in" everywhere (derived at display time, nothing stored). "No response" is the default and admin-only. Activity creation runs through the `/announce` wizard. |
